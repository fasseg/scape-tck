...
public class ConnectorAPIMockTest {
	...
	public void testRetrieveBitStream() throws Exception {
		...
		resp = CLIENT.execute(get);
		assertTrue(resp.getStatusLine().getStatusCode() == 200);
		BitStream fetched = MetsMarshaller.getInstance().deserialize(BitStream.class, resp.getEntity().getContent());
		get.releaseConnection();
		assertEquals(entity.getRepresentations().get(0).getFiles().get(0).getBitStreams().get(0), fetched); // check for
																											// some
																											// content
	}
	...
	public void testSearchEntity() throws Exception {
		...
		resp = CLIENT.execute(get);
		assertTrue(resp.getStatusLine().getStatusCode() == 200);
		IntellectualEntityCollection resultSet = (IntellectualEntityCollection) MetsMarshaller.getInstance().getJaxbUnmarshaller()
				.unmarshal(
	...
	public void testIngestMinimalIntellectualEntityAsynchronously() throws Exception {
		...
		resp = CLIENT.execute(get);
		LifecycleState lifecycle = (LifecycleState) MetsMarshaller.getInstance().getJaxbUnmarshaller()
